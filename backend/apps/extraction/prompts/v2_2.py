"""Extraction prompt v2.2 (v0.3 prayer-ready patch).

Same output schema and hard rules as v2.1. Extends standardized property
names so upcoming spaced-repetition and prayer-list features have stable keys.
"""
...
- `current_prayer_requests` — current prayer asks the person has shared.
- `current_stressors` — current stressors they are carrying.
- `upcoming_life_events` — upcoming events they are anticipating or dreading.
- `health_concerns` — health conditions, treatments, or medical worries.
- `family_concerns` — family burdens, conflicts, or caregiving concerns.
- `spiritual_state` — clearly stated current spiritual season.
- `life_goals` — goals they are currently pursuing.
- `unrealized_dreams` — long-range dreams not yet fulfilled.
- `greatest_fears` — fears directly stated by the entry text.
- `salvation_story` — testimony / conversion story details.
- `spiritual_gifts` — stated gifts or ministry strengths.
- `ministry_involvement` — ministry roles or service involvement not already
  represented via OrganizationMembership.
- `favorite_verse` — preferred Bible verse.
